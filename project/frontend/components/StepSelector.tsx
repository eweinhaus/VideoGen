...
export function StepSelector({ value, onChange, disabled }: StepSelectorProps) {
  ...
  return (
    <div className="space-y-3">
      <label htmlFor="stop_at_stage" className="text-sm font-medium">
        Stop After Stage (Testing Only)
      </label>
      ...
      <select
        ...
        value={value || ""}
        onChange={(e) => {
          const selectedValue = e.target.value as PipelineStage | ""
          onChange(selectedValue || null)
        }}
        disabled={disabled}
        className="w-full px-3 py-2 text-sm border rounded-md bg-background focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
      >
