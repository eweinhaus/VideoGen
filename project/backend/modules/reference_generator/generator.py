...
# Pinned to specific version (SDXL v1.0) - DO NOT use 'latest' tag
# Version: stability-ai/sdxl:39ed52f2-78e6-43c4-bc99-403f850fe245
...
def get_model_version() -> str:
    ...
    # Use same model for both dev and prod (consistency)
...
async def generate_image(
    ...
    try:
        ...
        # Handle output (may be a list or single URL)
        if isinstance(output, list):
            output_url = output[0] if len(output) > 0 else None
        else:
            output_url = output
...
async def generate_all_references(
    ...
    import os
    concurrency = int(os.getenv("REFERENCE_GEN_CONCURRENCY", "4"))
